...
class ShoppingCart extends Model {
    ...
    public function orderSearch($search,$pageSize,$page)
    {
        ...
        ("select * from orders where  mobile like '%$search%' or fullname like '%$search%' order by created_at desc limit $from,$pageSize");
        $obj_select->execute();
        $orders = $obj_select->fetchAll(PDO::FETCH_ASSOC);
        return $orders;
    }
    ...
    public function countorderSearch($search)
    {
        ...
        prepare("select count(orders.id) from orders where  mobile like '%$search%' or fullname like '%$search%'");
